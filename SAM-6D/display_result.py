...
def main():
    ...
    p.add_argument("--example-dir", default="./Data/Example",
    ...
    img = cv2.imread(os.path.join(args.example_dir,  "rgb.png"))
    cam = json.load(open(os.path.join(args.example_dir, "camera.json")))
    ply = os.path.join(args.example_dir, "obj_000001.ply")
    ...
    result_result_path = os.path.join(args.example_dir, 'outputs', "sam6d_results", "detection_pem.json")
    csv_result_path = os.path.join(args.example_dir, 'outputs',"predictions.csv")
 
    
    dets = json.load(open(result_result_path))

    if not dets:
        raise ValueError(f"No detections in {args.input}")

    # pick best
    best = max(dets, key=lambda d: d.get("score", 0.0))


    # prepare R and t fields as flat space-separated strings
    R = best["R"]   # a 3×3 list
    t = best["t"]   # a length-3 list

    R_flat = [f"{x:.8g}" for row in R for x in row]
    t_flat = [f"{x:.8g}" for x in t]
