...
async def estimate_pose(img_folder, image_id, object_type, scene_name):
    """
    Run SAM-6D inference and then load pose from detection_pem.json
    """
    ...
    # 2) locate the SAM-6D JSON
    print(f"[server] Searching Result for image {image_id}")
    csv_path = os.path.join(
        ".", "Data", "Example", "outputs", "predictions.csv"
    )
...
async def handler(ws):
    ...
    zero_pose = json.dumps({"position":[0,0,0], "rotation":[0,0,0,0,0,0,0,0,0]})
    try:
        while True:
            # 1) header
        ...
        try:
            await ws.send(zero_pose)
            print("[server] Sent zero‐pose on shutdown")
        except:
            pass
