...
def main(args):
    ...
    # Global intrinsics and image list
    global_K = None
    depth_scale = 1.0
    all_test_images = []
    gt_entries = {}
    ...
    for scene in scenes:
        ...
        # 3) Build per-image intrinsics dict, write scene_out/scene_camera.json
        ...
        # 4) Populate GT placeholders and test_targets list
        for im_id in img_ids:
    ...
    # 5) Write aggregated per-scene file under test_dir/scene_camera.json
    (test_dir / 'scene_camera.json').write_text(json.dumps(all_scene_cams, indent=2))

    # 6) Write global test_dir/camera.json using first scene intrinsics
    ...
    # 7) Write test_targets_bop19.json and other files as before
    test_targets = [
