...
def main():
    ...
    # load GT data
    gt = json.load(open(os.path.join(args.example_dir, "GT", "scene_gt.json")))
    img = cv2.imread(os.path.join(args.example_dir, "SAM6D", "rgb.png"))
    cam = json.load(open(os.path.join(args.example_dir, "SAM6D","camera.json")))
    ...
    result_result_path = os.path.join('debug', "ob_in_cam", "000000.txt")
    csv_result_path = os.path.join(args.example_dir, 'outputs',"predictions.csv")
 
    
    M = load_pose_txt(result_result_path)

    R = M[:3, :3].reshape(-1)   
    t = M[:3,  3]              
    t_list = [t2 * 1000 for t2 in t]
    ...
    with open(csv_result_path, "w", newline="") as f:
        ...
        t_str = " ".join(f"{v:.6f}" for v in t_list)
