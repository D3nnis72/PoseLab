...
def copy_test_data(example_root: Path, gigapose_root: Path, scene="000000", exp_gt='', exp_gt_info=''):
    ...
    files = {
        ...
        example_root / "image_rgb.png":
            lmo_root / "rgb" / f"{scene}.png",
        example_root / "image_depth.png":
            lmo_root / "depth" / f"{scene}.png",
        example_root / "camera.json":
...
def decode_rle(segmentation):
    """
    Decode COCO RLE (uncompressed list or compressed) segmentation to a boolean mask.
    """
    # segmentation: dict with 'counts' and 'size'
    counts = segmentation['counts']
    size = segmentation['size']  # [h, w]
    # If counts is a list, create RLE via frPyObjects
    if isinstance(counts, list):
        rles = mask_utils.frPyObjects(segmentation, size[0], size[1])
    else:
        # counts is a bytes or string
        rles = segmentation
    # Decode to mask; frPyObjects returns list of RLEs
    mask = mask_utils.decode(rles)
    # mask shape: (h, w) or (h, w, N)
    if mask.ndim == 3:
        mask = np.any(mask, axis=2)
    return mask.astype(bool)


def compute_modal_bbox(mask: np.ndarray) -> list:
    """
    Compute the tight bounding box [x_min, y_min, x_max, y_max] for a boolean mask.
    """
    ys, xs = np.where(mask)
...
def convert(input_path: str, output_path: str, label: str = 'barbecue-sauce'):
    """
    Read COCO-format detections with RLE segmentation at input_path,
    select the highest-scoring entry, compute its modal bbox, and write
    a simplified JSON [{"label": label, "bbox_modal": [...] }].
    """
    with open(input_path) as f:
        detections = json.load(f)

    # select entry with max score
    best = max(detections, key=lambda d: d.get('score', 0))
    seg = best.get('segmentation')
    mask = decode_rle(seg)
    bbox_modal = compute_modal_bbox(mask)
    ...
    print(f"Wrote modal bbox for label '{label}' to {output_path}")
...
def main():
    ...
    p.add_argument("--example_root", default="./local_data/examples/barbecue-sauce",  required=False,
    ...
    p.add_argument("--raw_models", default="../SharedData/ObjectModelsM",
    ...
    exp_megapose = os.path.join(args.export_root, "MegaPose")

    dst = args.example_root

    cad_ply = os.path.join(dst, "barbecue-sauce",args.cad)

    # 1) Copy photos
    print("\n==> Step 1: Copy photos")
    timings['copy_input_start'] = time.time()
    copy_tree(exp_megapose, dst)

    exp_camera_sam = os.path.join(args.export_root, "SAM6D", "camera.json")
    exp_camera_sam_dst = os.path.join(dst, "camera.json")
    copy_file(exp_camera_sam, exp_camera_sam_dst)

    exp_gt = os.path.join(args.export_root, "GT", "scene_gt.json")
    exp_gt_info = os.path.join(args.export_root, "GT", "scene_gt_info.json")
    exp_gt_dst = os.path.join(dst, "scene_gt.json")
    copy_file(exp_gt, exp_gt_dst)
    ...
    dst_foundationpose_model_dir = os.path.join(dst, "meshes", "barbecue-sauce", "hope_000002.ply")
    ...
    if args.test_all_primary == True:
        ...
        if args.novel:
            ...
            dst_model_dir = os.path.join(dst, "outputs")
    ...
    if args.test_all_primary == True:
        ...
        run([
            ...
            "--output_dir", os.path.join('..', dst, "outputs"),
            "--cad_path",    os.path.join('..', dst, "meshes", "barbecue-sauce", "hope_000002.ply"),
            "--rgb_path",    os.path.join('..', dst, "image_rgb.png"),
            "--depth_path",  os.path.join('..', dst, "image_depth.png"),
            "--cam_path",    os.path.join('..', dst, "camera.json"),
        ...
        dest_ism_result_file =  "./local_data/examples/barbecue-sauce/outputs/sam6d_results/detection_ism.json"
        copy_if_exists(ism_result_file, dest_ism_result_file)

        ism_mask_file = os.path.join(args.primary_result_file, 'Pose','MaskISM.png')
        dest_ism_mask_file =  "./local_data/examples/barbecue-sauce/outputs/sam6d_results/mask_ism.png"
        ...
        dest_ism_vis_file =  "./local_data/examples/barbecue-sauce/outputs/sam6d_results/vis_ism.png"
        copy_if_exists(ism_vis_file, dest_ism_vis_file)
        convert(f"{dest_ism_result_file}", f"./local_data/examples/barbecue-sauce/inputs/object_data.json", label="barbecue-sauce")
    ...
    copy_model_assets(raw_models, args.object_type, gigapose_root)
    ...
    result_dst = os.path.join("..", "Result", args.object_type, args.visibility, args.lighting, args.distance, args.height, scene_name, "MegaPose")
    ...
    result_dst = os.path.join(dst, "outputs")
    copy_if_exists(os.path.join(result_dst, "sam6d_results", "vis_ism.png"), os.path.join(vis_result_dst, "VisInstanceSegmentation.png"))

    pose_result_src = os.path.join(result_dst, "object_data.json")
    copy_if_exists(pose_result_src, os.path.join(pose_result_dst, "PoseEstimation.json"))
    

    sam_result_src = os.path.join(result_dst, "sam6d_results")
    ...
    score_result_src = os.path.join(result_dst, "predictions.csv")
