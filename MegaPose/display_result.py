...
def main():
    ...
    p.add_argument("--example-dir", default="./local_data/examples/barbecue-sauce",
    ...
    # load GT data
    gt = json.load(open(os.path.join(args.example_dir, "scene_gt.json")))
    img = cv2.imread(os.path.join(args.example_dir,  "image_rgb.png"))
    cam = json.load(open(os.path.join(args.example_dir, "camera.json")))
    ply = os.path.join(args.example_dir, "meshes", "barbecue-sauce", "hope_000002.ply")
    ...
    result_path = os.path.join(args.example_dir, 'outputs',"object_data.json")
    ...
    with open(csv_result_path, "w", newline="") as f:
        ...
        for entry in data:
            label = entry["label"]
            obj_id = OBJ_MAP.get(label, 0)
            score = entry.get("score", 0)
            time_ = entry.get("time", 0)


            R_list = entry["TWO"][0]
            t_list = entry["TWO"][1]
            t_list = [x * 1000 for x in t_list]

            print(entry["TWO"][1])

            if len(R_list) == 4:
                R9 = quat_to_rotm(R_list)
            elif len(R_list) == 9:
                R9 = R_list
            else:
                raise ValueError(f"Can't interpret R of length {len(R_list)}")

            R_str = " ".join(f"{x:.16g}" for x in R9)
            t_str = " ".join(f"{x:.16g}" for x in t_list)

            writer.writerow([0, 0, obj_id, score, R_str, t_str, time_])
