...
def copy_test_data(example_root: Path, gigapose_root: Path, scene="000000", exp_gt='', exp_gt_info=''):
    ...
    files = {
        ...
        example_root / "test" / "000000" / "rgb" / "000000.png":
            lmo_root / "rgb" / f"{scene}.png",
        example_root / "test" / "000000" / "depth" / "000000.png":
            lmo_root / "depth" / f"{scene}.png",
        example_root / "test" / "000000" / "camera.json":
            lmo_root / "camera.json",
        example_root / "test" / "000000" / "scene_gt.json":
            lmo_root / "scene_gt.json",
        example_root / "test" / "000000" / "scene_gt_info.json":
...
def write_scene_camera(export_root: Path, gigapose_root: Path, scene="000000"):
    rgb_dir = export_root / "0" / "FoundationPose" / "rgb"
    cam_txt = export_root  / "0" / "FoundationPose" / "cam_K.txt"
...
def main():
    ...
    p.add_argument("--export_root", default="../SharedData/WebsocketData/000000",   required=False,
                   help="Folder containing SAM6D raw: e.g. ./WebsocketData/")
    p.add_argument("--example_root", default="./Dataspace/lm",  required=False,
    ...
    p.add_argument("--object_type", default="brake",   required=False,
    ...
    run(["python", "convert_to_bopm.py",
         "--input_dir", args.export_root,
         "--output_dir", args.example_root])
    ...
    dst_source_dir = os.path.join(dst, "models")
    copy_tree(src_model_dir, dst_source_dir)
    dst_source_eval_dir = os.path.join(dst, "models_eval")
    copy_tree(dst_source_dir, dst_source_eval_dir)

    src_camera = os.path.join(dst, "test", "camera.json")
    dst_camera = os.path.join(dst, "camera.json")
    copy_file(src_camera, dst_camera)

    from PIL import Image
    rgb_image = os.path.join(dst, "test", "000000","rgb", "000000.png")
    img = Image.open(rgb_image).convert("RGB")
    img.save(rgb_image)

    print("\n==> Step 2: Copy ove6d template")
    timings['copy_input_end'] = time.time()

    timings['copy_templates_start'] = time.time()
    src_template_dir = os.path.join(args.raw_models, args.object_type, "ove6d_codebook")
    dst_template_dir = os.path.join(".", "evaluation", "object_codebooks")
    if os.path.isdir(dst_template_dir):
        shutil.rmtree(dst_template_dir)
    os.makedirs(dst_template_dir, exist_ok=True)
    copy_tree(src_template_dir, dst_template_dir)
    ...
    if args.test_all_primary == True:
        ...
        if args.novel:
            ...
            # 3) If existing model: copy raw_models/<type>/model → example_root
    ...
    if args.test_all_primary == True:
        ...
        run([
            ...
            "--output_dir", os.path.join('..','..','..', 'OVE6D-pose', dst, "outputs"),
            "--cad_path",    os.path.join('..','..','..', 'OVE6D-pose', dst, "models", "obj_000001.ply"),
            "--rgb_path",    os.path.join('..','..','..', 'OVE6D-pose', dst,  "test", "000000", "rgb", "000000.png"),
            "--depth_path",  os.path.join('..','..','..','OVE6D-pose', dst, "test", "000000", "depth", "000000.png"),
            "--cam_path",    os.path.join('..','..','..','OVE6D-pose', dst,  "test", "000000","camera.json"),
        ], cwd="../SAM-6D/SAM-6D/Instance_Segmentation_Model")
        timings['instance_segmentation_end'] = time.time()
    else:
        print("\n==> Step 4: Copy Segmentation Result")
        ism_result_file = os.path.join(args.primary_result_file, 'Pose','InstanceSegmentation.json')
        dest_ism_result_file =  "./Dataspace/lm/outputs/sam6d_results/detection_ism.json"
        copy_if_exists(ism_result_file, dest_ism_result_file)

        ism_mask_file = os.path.join(args.primary_result_file, 'Pose','MaskISM.png')
        dest_ism_mask_file =  "./Dataspace/lm/outputs/sam6d_results/mask_ism.png"
        ...
        dest_ism_vis_file =  "./Dataspace/lm/outputs/sam6d_results/vis_ism.png"
    ...
    copy_test_data(Path(dst), gigapose_root, "000000")
    ...
    result_dst = os.path.join("..", "Result", args.object_type, args.visibility, args.lighting, args.distance, args.height, scene_name, "OVE6D")
    ...
    img_result_src = os.path.join("Dataspace", "lm", "test", "000000" )
    ...
    copy_if_exists(os.path.join(result_src, "sam6d_results", "vis_ism.png"), os.path.join(vis_result_dst, "VisInstanceSegmentation.png"))
    
    pose_result_src = os.path.join(result_src,  "sam6d_results", "detection_pem.json")
    copy_if_exists(pose_result_src, os.path.join(pose_result_dst, "PoseEstimation.json"))

    pose_result_src = os.path.join(result_src,  "sam6d_results", "detection_ism.json")
    copy_if_exists(pose_result_src, os.path.join(pose_result_dst, "InstanceSegmentation.json"))
    
    pose_result_src = os.path.join(result_src,  "sam6d_results", "mask_ism.png")
